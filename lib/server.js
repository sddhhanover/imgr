...
Server.prototype.resizeImage = function (src, dest, parameters, callback) {
    ...
    if (src.match(/^(https?|ftp):\/\/[^\s\/$.?#].[^\s]*$/)) {
        //Create the output dir if it doesn't already exist
        mkdirp(dir, function () {
            request(src).on('response', function(resp) {
                if (resp.statusCode != 200) { return callback(); };
                self.resizeImage(dest + '_tmp', dest, parameters, callback);
            }).pipe(fs.createWriteStream(dest + '_tmp'));
        });
        return;
    }
...
Server.prototype.middleware = function () {
    ...
    function tryContentDirectory(req, res, next) {
        info('trying to serve %s', path.join(base_dir, req.url));
        //Regex from @stephenhay
        if (base_dir.match(/^(https?|ftp):\/\/[^\s\/$.?#].[^\s]*$/)) {
            request.head(base_dir + req.url, function(err, resp) {
                if (!err && resp.statusCode == 200) {
                    request.get(base_dir + req.url).pipe(res).on('close', function(response) {
                        next();
                    });
                }
                else {
    ...
    function resizeImage(request, response, next) {
        ...
        var src_image = base_dir + '/' + decodeURIComponent(parameters.path)
